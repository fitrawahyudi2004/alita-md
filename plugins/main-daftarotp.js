...
let handler = async function (m, { text, args,  usedPrefix, command }) {
    ...
    if (user.registered === true) return m.reply('Akun Kamu Sudah Terverifikasi dan terdaftar di *DATABASE* Alita-MD')
     let anus = (`\nKode OTP anda adalah\n\nJangan Berikan kepada Siapapun Maupun Owner!\n\nBalas Pesan ini menggunakan kode tersebut harap perhatikan besar kecil huruf\n\n${wm}`)
...
handler.all = async m => {
    ...
    if (m.text == conn.verify[m.sender].code && m.quoted.id == conn.verify[m.sender].key) {
        conn.sendFile(m.chat, flas, 'nih.jpg', `Serial Number kamu : ${sn}\n\n*Terima Kasih karena sudah memverifikasi akun anda di DATABASE Alita-MD*\n\nHarap bintangi pesan ini agar kamu mudah melakukan Unregister\n\n${wm}`, m)
